...
public class SpendingEvolutionChartData : IDisposable
{
    ...
    public SpendingEvolutionChartData()
    {
        XAxes[0] =
            new Axis
            {
    ...
    public void RefreshChart(SpendingEvolutionDataDto data)
    {
        FiatCurrency = FiatCurrency.GetFromCode(data.PrimaryCurrency);
        FiatValues.Clear();
        BtcValues.Clear();
        MonthLabels.Clear();
        ...
        // after a window resize combined with a dropdown change.
        DisposeSeries();
        Series.Clear();
        ...
        for (var index = 0; index < data.Months.Count; index++)
        {
            ...
            BtcValues.Add(new ObservablePoint(index, (double)month.SatsTotal / 100_000_000.0));
        }
