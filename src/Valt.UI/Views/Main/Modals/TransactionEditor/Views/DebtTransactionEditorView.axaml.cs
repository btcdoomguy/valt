...
public partial class DebtTransactionEditorView : UserControl, ITransactionEditorChildView
{
    private AutoCompleteBox? _autoCompleteTransactionNameBox;
    private BtcInput? _fromBtc;
    private FiatInput? _fromFiat;

    public DebtTransactionEditorView()
    {
        InitializeComponent();
    ...
    {
        if (DataContext is DebtTransactionEditorViewModel { FromAccountIsBtc: true })
        {
            _fromBtc?.Focus();
        }
        else
    ...
    private void AutoCompleteTransactionNameBox_OnDropDownClosed(object? sender, EventArgs e)
    {
        ...
        if (DataContext is not DebtTransactionEditorViewModel { FromAccount: not null }) return;
