...
public partial class CreditTransactionEditorView : UserControl, ITransactionEditorChildView
{
    private AutoCompleteBox? _autoCompleteTransactionNameBox;
    private BtcInput? _fromBtc;
    private FiatInput? _fromFiat;

    public CreditTransactionEditorView()
    {
        InitializeComponent();
    ...
    {
        if (DataContext is CreditTransactionEditorViewModel { FromAccountIsBtc: true })
        {
            _fromBtc?.Focus();
        }
        else
    ...
    private void AutoCompleteTransactionNameBox_OnDropDownClosed(object? sender, EventArgs e)
    {
        ...
        if (DataContext is not CreditTransactionEditorViewModel { FromAccount: not null }) return;
