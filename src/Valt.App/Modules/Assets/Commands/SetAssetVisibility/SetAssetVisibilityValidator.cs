using Valt.App.Kernel.Validation;

namespace Valt.App.Modules.Assets.Commands.SetAssetVisibility;

internal sealed class SetAssetVisibilityValidator : IValidator<SetAssetVisibilityCommand>
{
    public ValidationResult Validate(SetAssetVisibilityCommand instance)
    {
        var builder = new ValidationResultBuilder();

        builder.AddErrorIfNullOrWhiteSpace(instance.AssetId, nameof(instance.AssetId), "Asset ID is required.");

        return builder.Build();
    }
}
