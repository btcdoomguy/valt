...
[TestFixture]
public class MarkGoalsStaleEventHandlerTests
{
    private MarkGoalsStaleEventHandler _handler = null!;
    ...
    private ILogger<MarkGoalsStaleEventHandler> _logger = null!;
    ...
    public void SetUp()
    {
        ...
        _goalRepository.MarkGoalsStaleForDateAsync(Arg.Any<DateOnly>()).Returns(Task.CompletedTask);
        ...
        _logger = Substitute.For<ILogger<MarkGoalsStaleEventHandler>>();
        _handler = new MarkGoalsStaleEventHandler(_goalRepository, _progressState, _manager);
    }
    ...
    [Test]
    public async Task HandleAsync_TransactionCreatedEvent_MarksStaleThenTriggersJob()
    {
        var transaction = TransactionBuilder.ATransaction().AsFiatExpense(100m).BuildDomainObject();

        await _handler.HandleAsync(new TransactionCreatedEvent(transaction));
        await _manager.TriggerJobAndWaitAsync(BackgroundJobSystemNames.GoalProgressUpdater);

        await _goalRepository.Received(1).MarkGoalsStaleForDateAsync(transaction.Date);
    ...
    [Test]
    public async Task HandleAsync_TransactionEditedEvent_MarksStaleThenTriggersJob()
    {
        var transaction = TransactionBuilder.ATransaction().AsFiatExpense(100m).BuildDomainObject();

        await _handler.HandleAsync(new TransactionEditedEvent(transaction));
        await _manager.TriggerJobAndWaitAsync(BackgroundJobSystemNames.GoalProgressUpdater);
    ...
    [Test]
    public async Task HandleAsync_TransactionDeletedEvent_MarksStaleThenTriggersJob()
    {
        var transaction = TransactionBuilder.ATransaction().AsFiatExpense(100m).BuildDomainObject();

        await _handler.HandleAsync(new TransactionDeletedEvent(transaction));
        await _manager.TriggerJobAndWaitAsync(BackgroundJobSystemNames.GoalProgressUpdater);
